...
> 前提是已安装pipx管理器，未安装看之前内容。优点是可以避免pip与apt包管理器冲突

> 安装主程序

```
# pipx安装glances, 成功自动链接到 ~/.local/bin 目录中。
...
# Web模式设置登录用户名/密码， 默认用户名（glances）
## 输入指令后连续输入两次用户名/密码，再输入"Yes"确认保存，保存位置~/.config/glances/<username>.pwd
glances -w --username
glances -w --password
```
...
> 前提是需要提前安装python3-venv虚拟环境，优点是可解决“pip install”提示“apt”和“pip”包管理器冲突，适合pipx无法安装时使用。
...
deactivate
# 创建软连接,仅添加到当前的用户变量中。
sudo ln -s ~/software_home/glances/bin/glances ~/.local/bin/
```
...
> 包管理器安装没有WebUI，不适用此方法。
>
> 关于启用WebUI密码，需要添加systemd服务前设置号登录密码，默认账户名glances；不启用可忽略。

> 编辑systemd文件；添加代码块中的内容
>
> 参数："test"改为真实用户名；"8398"是WebUI访问端口；"--password"启用web密码，不启用可删；"-t"刷新数据的间隔时间，此处为2秒。
...
User=test
# 不启用密码，删除参数"--passwor"
ExecStart=/home/test/.local/bin/glances -w -p 8398 -t 2 --password
Restart=on-abort
...
> 使用"ctrl + w"检索"blowfish_secret"，定位到代码块中的内容，并将[Blowfish生成器](https://phpsolved.com/phpmyadmin-blowfish-secret-generator/)生成的32 字符随机字符串填入单引号内。
...
> 创建”aria2.conf“文件，存放路径可自定义，例如："~/software_home/aria2_data/aria2.conf"。填写精简版的内容。
...
> 创建"aria2.log"日志文件和”aria2.session“文件，与”aria2.conf“文件路径保持一致
>
> 例："~/software_home/aria2_data/aria2.session"。
...
chown test:test ~/software_home/aria2_data/aria2.log ~/software_home/aria2_data/aria2.session
# 赋予权限："600"为只有拥有者有读写权限
...
> 填入下方代码块的内容；
>
> 参数：”User=test“ 删除则使用root用户运行，个人不喜欢用root用户运行，填写了test；"--conf-path="后填入"aria2.conf"存放的路径。
...
# 解压"AriaNg"至网页根目录/var/www/
sudo unzip AriaNg-1.3.7.zip -d /var/www/ariaNG

# 赋予目录权限755，并更改用户和组为www-data
...
> 免设置登陆（推荐），可配合导航页使用。
>
> 参数：<ip>为真是IP地址；<Base64_rpc-key>为经过Base64编码的rpc口令，[Base64 计算器](https://www.toolhelper.cn/EncodeDecode/Base64) 
...
> Tips：若出现修改端口后，下载速度很低或者没有速度，在“设置->连接->监听端口”处重新设置一个随机端口口。
...
> 编辑 "qbittorrent-nox.service"，填入代码块的内容。其中的参数：“User=test“为真实用户名；”ExecStart=”为解压的二进制文件路径，"-d"表示后台运行。
...
Type=forking
#RemainAfterExit=yes
ExecStart=/home/test/software_home/qbittorrent_home/qbittorrent-nox -d
Restart=always
RestartSec=10
...
> Tracker地址1：[XIU2/TrackersListCollection](https://github.com/XIU2/TrackersListCollection/blob/master/README-ZH.md) 
>
> Tracker地址2：[trackerslist:BitTorrent trackers](https://github.com/ngosang/trackerslist?tab=readme-ov-file) 
>
> Tracker地址3：[稳定跟踪器列表 - newTrackon](https://newtrackon.com/list) 
>
> Tracker地址4：[中国可用的 BT Tracker 服务器列表，每24小时自动更新 ](https://www.yaozuopan.top/index.php/archives/1014/)

#### WebUI（皮肤）

VueTorrent

简介：VueTorrent使用 Vue.js 制作的 qBittorrent 美观的 WebUI，是Github上的一款开源的qBittorrent主题，支持多语言、深色和浅色主题，支持移动端查看，设置功能支持的也很全面，界面、行为、下载、连接、速度、BT、RSS、WebUI、标签和分类与原版别无二致。

![image-20241023095821792](https://cdn.jsdelivr.net/gh/GKK2024/Convert-an-NC10-into-a-NAS@main/Images/202410230958279.png)

主题地址：[vuetorrent.zip](https://github.com/VueTorrent/VueTorrent/releases/download/v2.15.0/vuetorrent.zip) | [Releases](https://github.com/VueTorrent/VueTorrent/releases) | [Github](https://github.com/VueTorrent/VueTorrent) 
...
[qbittorrent-nox的安装，开机启动和常用命令](https://www.tofzl.com/index.php/archives/19/) 

[Ubuntu安装qbittorrent-nox](https://www.shiyanhe.com/166.html/) 
...
> 先停止服务，再修改transmission-daemon配置文件，避免被覆盖；

```
sudo systemctl stop transmission-daemon.service
```

> 编辑“settings.json”；必要修改，通过"ctrl + w"检索
...
> 简介："transmission-web-control"是一款定制版的Web UI，为Transmission提供了一个功能丰富且易用的增强界面；不仅允许您添加种子文件或URL，还支持拖放操作来上传种子。它包含了对Transmission设置的在线修改功能，如下载目录、速度限制和端口等；详解：[Home CN · Github](https://github.com/ronggang/transmission-web-control/wiki/Home-CN)。
...
> 目前最新的4.0.6版本，需要通过源码安装；
>
> 程序包：[源码包 ：transmission-4.0.6.tar.gz](https://github.moeyy.xyz/https://raw.githubusercontent.com/GKK2024/Convert-an-NC10-into-a-NAS/refs/heads/main/note-files/transmission-4.0.6.tar.gz)  |  [主题包：theme-transmission.tar.gz](wget https://github.moeyy.xyz/https://raw.githubusercontent.com/GKK2024/Convert-an-NC10-into-a-NAS/refs/heads/main/note-files/theme-transmission.tar.gz) 
...
wget https://github.moeyy.xyz/https://raw.githubusercontent.com/GKK2024/Convert-an-NC10-into-a-NAS/refs/heads/main/note-files/transmission-4.0.6.tar.gz
```
...
> 首次启动会产生配置为文件到用户目录下"~/.config/transmission/settings.json"，需要编辑这个文件才能通过网页访问，并且更改设置需要在程序关闭状态下才能生效。
...
> 以下是 "settings.json" 需要的内容，使用"ctrl + w" 检索定位，修改后使用"ctrl + x"保存退出。

```
# 访问是否需要密码，true & false；"false"关闭后访问WebUI不需要密码
 "rpc-authentication-required": true,
...
# 是否开启IP 地址白名单，true & false；"false"关闭后所有IP可访问WebUI，访问WebUI时"403"则需要关闭；
 "rpc-whitelist-enabled": true,
...
> 只需要将安装位置和运行用户对应的内容修改，并拷贝到systemd目录。

```
nano ~/software_home/transmission/lib/systemd/system/transmission-daemon.service
```

```
# "User"后填"%i",会根据systemd的服务名调整而变化。
User=%i
# 将路径修改位"transmission-daemon"的真实路径。
ExecStart=/home/test/software_home/transmission/bin/transmission-daemon -f --log-error
```

> 拷贝到systemd目录，并给服务添加"@<name>"，"<name>"填写真实用户名，如 ”@test“。

```
sudo cp ~/software_home/transmission/lib/systemd/system/transmission-daemon.service /etc/systemd/system/transmission-daemon@test.service 
```
...
```
http://<ip>:port
例：http://192.168.1.54:9091
```

添加主题(皮肤)

>v4.0.6的主题目录名有所变化，无法使用脚本安装，可以直接使用文件进行覆盖。
...
# 创建目录"alist_home"
mkdir ~/software/alist_home
# 解压"alist"到"alist_home"：
...
http://<ip>:5244
## 默认-用户名：admin
	  -密 码：首次启动可见，忘记看后文"关于命令行修改密码"
```

配置文件及必要修改项

> 编辑配置文件，通过"ctrl + w"检索必要修改项
...
User=test
Type=simple
WorkingDirectory=/home/test/software/alist_home
ExecStart=/home/test/software/alist_home/alist server
Restart=on-failure
...
> **关于"发送到Aria2"和侧栏"使用Aria2离线下载"的区别**
>
> 发送到Aria2：直接使用aira2下载默认下载位置。需提前在侧栏"本地设置"中填写"Aria2 RPC 链接"和"Aria2 RPC 密钥"。
>
> ![image-20241024122821154](https://cdn.jsdelivr.net/gh/GKK2024/Convert-an-NC10-into-a-NAS@main/Images/202410241228712.png)
>
> 侧栏-使用Aria2离线下载：先离线下载，然后在上传到指定的网盘/目录中；具体操作是在alist网页端中，自行进入需指定的下载目录后，通过侧栏"离线下载"选择"aria2"，粘贴下载链接后进行下载。如需要保留上传的文件则增加选择“从不删除”。
>
...
# 添加新用户和密码，参数"--perm.admin"表示设置为管理员账户；"root" "password" 分别为需要设置的新用户名和密码。
...
[Service]
User=test
ExecStart=/usr/local/bin/filebrowser -d /home/test/software/filebrowser_home/filebrowser_data/filebrowser.db
# 手动安装使用下方命令：
# ExecStart=/home/test/.local/bin/filebrowser -d /home/test/software/filebrowser_home/filebrowser_data/filebrowser.db
Restart=always
RestartSec=10
...
> 配置数据库；最简单的是使用"SQLite + 文件缓存"组合，其次是"MySQL + 文件缓存"，二择一即可。
>
...
```
1. "Running setup.py install for iso-639 did not run successfully."
...
2. "Building wheel for netifaces (pyproject.toml) ... error"
	需要重新安装netifaces库：pip3 install netifaces
```

添加systemd自启动服务

> 编辑calibre-Web@<name>.service，将"<name>"改为真实用户名，如 test；
...
为**"calibre-web"**网页端添加转换器

> 1、登录网页登录管理员账户；
>
> 2、右上角进入"管理权限"  -->  "编辑基本配置"  -->  "扩展程序配置"  --->  选择二进制文件具体路径
>
...
> 进入用户目录，下载 "阅读3" 服务器版 "reader-server"
...
> 在reader-server的 v3.2.7 ~ v3.2.10 版本都有 startup.sh 脚本启动不了的情况
>
> 解决方法是修改 "start.sh" 脚本，"ctrl + w" 检索 "grep -EO"
...
> 编辑 "lanraragi-systemd.service"

```
nano ~/software_home/lanraragi/tools/lanraragi-systemd.service
```

> 替换 [Service] 下对应行代码为以下命令

```
[Service]
# 添加一行执行用户代码
User=%i
# 端口自定义，文件默认80，此处设为3001
Environment=LRR_NETWORK=http://*:3001
# 工作目录设置为安装程序的路径
WorkingDirectory=/home/%i/software_home/lanraragi
ExecStart=/usr/bin/perl /home/%i/software_home/lanraragi/script/launcher.pl -f /home/%i/software_home/lanraragi/script/lanraragi
```

> 复制到 systemd 目录，将"@<name>"后的用户名，如test 修改该为真实用户名。
...
> Ubooquity原版为英文且持续更新，需要手动下载上传Nas；根据跟人喜好选择原版或者汉化版即可。
>
> 进入用户目录，下载Ubooquity
...
> 解压Ubooquity至"Ubooquity_home"目录
...
> 编辑"ubooquity@<name>.service"，将"<name>"改为真实用户名
...
sudo systemctl status ubooquity.service
```
...
# 创建 "Suwayomi-Server_home"
mkdir -p ~/software_home/suwayomi-Server_home
```

> 下载 Suwayomi 程序 java 版
...
> 参数说明："-Dsuwayomi.tachidesk.config.server.rootDir"：指定工作目录
...
> 编辑 "suwayomi@<name>.service" ，将<name>修改位真实用户名；
...
> WebUI 登陆默认端口：4567；更改端口需要修改配置文件"server.conf".
...
> （1）侧边栏：设置-->浏览-->扩展库；
>
> （2）添加源：https://github.moeyy.xyz/https://github.com/keiyoushi/extensions/tree/repo
>
> （3）侧边栏：浏览-->插件；
>
> 扩展设置2：本地安装插件；
>
> （1）插件下载：[Extensions · 列表](https://keiyoushi.github.io/extensions/) 
>
> （2）侧边栏：浏览-->插件-->点击 " + " 安装外部插件；
>
...
>  通过云盘下载 "kkFileView-4.4.0-beta_linx.tar.gz"，使用scp等工具上传至用户目录 " ~/ "；
...
```
# 通过 "ctrl + w"组合键，检索 "KK_OFFICE_HOME" ,定位 "office.home"，然后修改为下方的内容
...
> 编辑 "kkFileView@<test>.service"，将"<name>"改为真实用户名，如 test；
...
mkdir -p ~/software_home/navidrome_home/config_dir
mkdir -p ~/software_home/navidrome_home/cache
mkdir -p ~/software_home/navidrome_home/music
# 解压至 "navidrome_home"
tar -zxvf navidrome_0.53.3_linux_386.tar.gz -C ~/software_home/navidrome_home
```

> 创建配置文件 "navidrome.toml"，注意修改存储目录的路径信息。
...
```
# 从外部的配置文件加载配置，默认为'./navidrome.toml'
...
> 编辑"navidrome@<name>.service"，将"<name>"改为真实用户名，如 test，用于运行程序；
...
# 安装主题：进入管理员后台-->控制台-->外观--可以使用的外观-->启用
```
...
```
# 创建目录 "dh_homePage"
sudo mv ./Home /var/www/dh_homePage
# 解压程序包
unzip ./Home.zip -d /var/www/dh_homePage
```
...
> 添加新图标：
>
> 1、下载" [Home导航页·图标](https://github.com/GKK2024/Convert-an-NC10-into-a-NAS/tree/main/note-files/Home_Program-icon)  " 加入图标目录。
>
> 2、存放图标目录" /var/www/dh_homePage/img/png "

> 添加导航地址：编辑 "index.html"，通过"ctrl + w"定位需要修改的内容；
...
> 编辑"van-nav-Page@<name>.service"；将"<name>"改为真实用户名，如 test；
...
# 解压
tar zxf flare_linux_386.tar.gz -C ~/software_home/flare
```

配置systemd服务

> 编辑 "flare-Page@test.service"，将 "<name>"改为真实用户名，如test。
...
> 环境要求：PHP5.4及以上(不建议使用PHP8)、MySQL5.5及以上；
...
![6](https://cdn.jsdelivr.net/gh/GKK2024/Convert-an-NC10-into-a-NAS@main/Images/202411050019009.png)
