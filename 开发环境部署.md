...
&emsp;其实，OMV系统默认环境是LNMP，即 Linux、Nginx、MySQL、PHP，它已经在脚本安装时帮助我们安装了部分环境，OMV 7的软件默认版本是nginx1.22、php8.2、php8.2-fpm，但缺少了MySQL数据库需要我们自己补充上。后文记录了各种运行环境的部署方法、注意事项，并非所有语言环境都要安装，主要取决于你使用的软件是否有要求。
